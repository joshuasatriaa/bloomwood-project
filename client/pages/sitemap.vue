<template>
  <div class="container mx-auto text-primary pt-20">
    <div class="md:ml-14">
      <h1 class="text-5xl sm:text-6xl md:text-7xl text-primary mb-20">
        Find Us
      </h1>
      <div class="grid grid-cols-2 gap-x-16">
        <div class="border-4 border-primary rounded-lg">
          <iframe
            class="w-full rounded"
            src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d14870.317741588633!2d106.76151294983356!3d-6.106555886267865!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x2e69edc0e6940113%3A0xf082213377c53ac1!2sBloomwood%20Florist!5e0!3m2!1sen!2sid!4v1630473156108!5m2!1sen!2sid"
            width="800"
            height="540"
            style="border: 0"
            allowfullscreen=""
            loading="lazy"
          />
        </div>
        <div class="text-primary mt-7">
          <h2 class="text-4xl font-bold mb-8">The Location</h2>
          <p class="text-2xl ml-7 pl-8 border-l-4 border-primary mb-10">
            Jalan Jenderal Sudirman Nomor 32, Kebon Jeruk, Jakarta Barat,
            Jakarta Barat 14400
          </p>
          <h2 class="text-4xl font-bold mb-8">Opening Hours</h2>
          <p class="text-2xl ml-7 pl-8 border-l-4 border-primary">
            Monday - Friday 10:00 - 18:00 WIB
            <br />
            Saturday - Sunday 11:00 - 19:00 WIB
          </p>
        </div>
      </div>
    </div>
  </div>
</template>
<script>
export default {}
</script>
<style></style>
